...
static ssize_t my_read(struct file *file_pointer, char __user *buffer, size_t count, loff_t *offset) {
    ...
    if (is_buffer_empty()) {
        ...
        // Wait until data is available or signal received
...
static void __exit my_module_exit(void) {
    if (circular_buffer) {
        // Set capacity to 0 to unblock any waiting threads before freeing
        // This is a safety measure to ensure threads waiting on buffer status wake up
        // and handle module unload gracefully.
        current_buffer_capacity = 0;
        wake_up_interruptible(&read_queue);
        wake_up_interruptible(&write_queue);
        // Give a very small moment for threads to react if needed
        // Not strictly necessary as kfree will eventually cause fault if they access,
        // but helps with clean unblock.
        // usleep_range(1000, 2000); // 1-2 ms (optional, for very rapid unload)
        kfree(circular_buffer);
...
MODULE_DESCRIPTION("A simple character device driver with circular buffer, blocking I/O, and dynamic capacity.");
MODULE_VERSION("1.1");
