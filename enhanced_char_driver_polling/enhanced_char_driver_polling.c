...
#include <linux/poll.h>     // <--- NEW: For poll_table, poll_wait, POLLIN, POLLOUT
...
// --- NEW: my_poll function for polling/select ---
static __poll_t my_poll(struct file *file, poll_table *wait) {
    __poll_t mask = 0;

    // Add current process to the wait queues
    // This function doesn't block, it just registers the wait queue.
    poll_wait(file, &read_queue, wait);
    poll_wait(file, &write_queue, wait);

    // Check for read readiness (POLLIN)
    if (!is_buffer_empty()) {
        mask |= POLLIN | POLLRDNORM; // POLLRDNORM is normal data for reading
    }

    // Check for write readiness (POLLOUT)
    if (!is_buffer_full()) {
        mask |= POLLOUT | POLLWRNORM; // POLLWRNORM is normal data for writing
    }

    // Optionally, add POLLERR if any critical error occurs
    // mask |= POLLERR;
...
static const struct file_operations my_fops = {
    ...
    .poll    = my_poll, // <--- NEW: Add the poll operation
};
...
MODULE_DESCRIPTION("A simple character device driver with circular buffer, blocking I/O, dynamic capacity, and polling.");
MODULE_VERSION("1.2");
