...
#include <poll.h>     // <--- NEW: For poll() system call
...
static bool use_polling_mode = false;   // <--- NEW: Flag to control polling mode
...
void *data_producer_thread(void *arg) {
    ...
    struct pollfd pfd; // <--- NEW: For polling mode
    pfd.fd = dev_fd;
    ...
    while (app_running) {
        // --- NEW: Polling logic ---
        if (use_polling_mode) {
            // Poll with a timeout (e.g., 100ms)
            ret = poll(&pfd, 1, 100); // 100ms timeout
            if (ret < 0) {
                if (errno == EINTR) {
                    printf("[Producer-Poll] Poll interrupted by signal. Exiting.\n");
                    goto producer_exit;
                }
                perror("[Producer-Poll] Error during poll");
                goto producer_exit;
            } else if (ret == 0) {
                // Timeout, no events
                printf("[Producer-Poll] No data available (timeout). Retrying...\n");
                continue; // Loop again to poll
            }
            // If ret > 0, an event occurred
            if (!(pfd.revents & POLLIN)) {
                // Not POLLIN, something else happened (e.g., POLLERR, POLLHUP)
                printf("[Producer-Poll] Unexpected poll event 0x%x. Exiting.\n", pfd.revents);
                goto producer_exit;
            }
        }
        // --- End Polling logic ---
        ...
        if (bytes_read < 0) {
            ...
            } else if (errno == EAGAIN) { // Non-blocking, no data yet (shouldn't happen with blocking reads unless poll missed it)
                // If using polling, EAGAIN here means a race condition or poll false positive
                // With blocking reads, this should typically not be hit.
                printf("[Producer] Read returned EAGAIN. (Likely race or poll timeout, not critical)\n");
        ...
        if (!use_polling_mode) { // Only sleep if not in polling mode, polling handles its own "waiting"
...
void display_usage(char *prog_name) {
    ...
    printf("  -r, --run                 Start producer/consumer threads and run application (blocking I/O).\n");
    printf("  -L, --poll-mode           Run producer/consumer threads using poll() for I/O readiness.\n"); // <--- NEW: Poll mode option
...
int main(int argc, char *argv[]) {
    ...
    // For poll() to work, O_NONBLOCK is NOT typically set. Poll() handles blocking.
    ...
    static struct option long_options[] = {
        ...
        {"run", no_argument, 0, 'r'},
        {"poll-mode", no_argument, 0, 'L'}, // <--- NEW: Long option for poll mode
    ...
    int opt;
    int long_index = 0;
    while ((opt = getopt_long(argc, argv, "scerhP:C:S:L", long_options, &long_index)) != -1) { // <--- NEW: Add 'L' to optstring
        switch (opt) {
            case 's':
                query_kernel_status();
                break;
            ...
            case 'r':
                run_threads = true;
                use_polling_mode = false; // Ensure blocking mode if --run is explicit
                break;
            case 'L': // <--- NEW: Handle poll mode option
                run_threads = true;
                use_polling_mode = true;
                printf("[User App] Polling mode enabled.\n");
                break;
            case 'P':
    ...
    // If no specific run command (--run or --poll-mode) given, default to running threads in blocking mode
    if (!run_threads && !exit_immediately) {
        run_threads = true;
        use_polling_mode = false; // Default to blocking if no explicit mode set
    }
    ...
    if (run_threads) {
        ...
        // Create consumer thread (consumer thread doesn't use poll in this example, only producer)
